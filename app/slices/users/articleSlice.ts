...
export const getArticles = createAsyncThunk(
  ...
  async (params: Record<string, any> | undefined, { dispatch, rejectWithValue }) => {
    try {
      const response: AxiosResponse = await axiosObj.get(`/api/users/articles`);
...
export const getArticle = createAsyncThunk(
  ...
  async (id: string, { dispatch, rejectWithValue }) => {
    try {
      const response: AxiosResponse = await axiosObj.get(`/api/users/articles/${id}`);
...
export const getnewArticleData = createAsyncThunk(
  "article/getnewArticleData",
  async (id: Record<string, any> | null, { dispatch, rejectWithValue }) => {
    try {
      const response: AxiosResponse = await axiosObj.get(`/api/users/articles/new`);
      if (response && response.status !== 200) {
        throw new Error(`HTTP error ${response.status}`);
      }
      const data = await response.data;
...
export const createArticle = createAsyncThunk(
  ...
  async (form: NewArticle,  { dispatch, rejectWithValue }) => {
    try {
      const response: AxiosResponse = await axiosObj.post(`/api/users/articles`, form);
      if (response && response.status !== 200) {
        throw new Error(`HTTP error ${response.status}`);
      }
      const data = await response.data;
...
export const updateArticle = createAsyncThunk(
  ...
  async ({ id, form }: UpdateArticle,  { dispatch, rejectWithValue }) => {
    try {
      ...
      const response: AxiosResponse = await axiosObj.put(`/api/users/articles/${id}`, form);
      if (response && response.status !== 200) {
        throw new Error(`HTTP error ${response.status}`);
      }
...
export const deleteArticle = createAsyncThunk(
  ...
  async (id: number,  { dispatch, rejectWithValue }) => {
    try {
      const response: AxiosResponse = await axiosObj.delete(`/api/users/articles/${id}`);
      if (response && response.status !== 200) {
        throw new Error(`HTTP error ${response.status}`);
      }
      const data = await response.data; // ✅ fetch returns a Response
...
const articleSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    .addCase(createArticle.fulfilled, (state, action: PayloadAction<{ article: Article}>) => {
     ...
     .addCase(updateArticle.fulfilled, (state, action: PayloadAction<{ article: Article}>) => {
      state.article = action.payload.article;
      // for (const [key, value] of Object.entries(action.payload)) { state[key] = value; }
    })
  }
